...
@Database(entities = {Booking.class, BookingContainMenu.class, Customer.class, MenuItem.class, Restaurant.class, Table.class}, version = 3, exportSchema = false)
    ...
    public static TableBookingDatabase getDatabase(final Context context) {
        if (INSTANCE == null) {
            synchronized (TableBookingDatabase.class) {
                if (INSTANCE == null) {
                    INSTANCE = Room.databaseBuilder(context.getApplicationContext(), TableBookingDatabase.class, "table_booking_database").fallbackToDestructiveMigration().allowMainThreadQueries().build();
                }
            }
        }
        return INSTANCE;
